...
func newGenerateInfo(modelFile, svcPath string, forceService bool) *generateInfo {
	...
	waitersFile := strings.Replace(modelFile, ".normal.json", ".waiters.json", -1)
...
func (g *generateInfo) writeWaitersFile() {
	...
	file := filepath.Join(g.PackageDir, "waiters.go")
	data := []byte(note + "package " + g.API.PackageName() + "\n\n" + g.API.WaitersGoCode())
...
func (g *generateInfo) writeAPIFile() {
	...
	code := fmt.Sprintf(note+"// Package %s provides a client for %s.\n"+
